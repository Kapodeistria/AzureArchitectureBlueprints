...
echo "🚀 Auto-Deploying to Azure AI Foundry Project"
echo "==========================================="
...
# Function to create and deploy agent flow
deploy_agent_flow() {
    local agent_name=$1
    
    echo -e "${YELLOW}📦 Creating and deploying $agent_name flow...${NC}"
...
EOF
    ...
    cd "flows/${agent_name}"
    echo -e "${BLUE}  🔄 Deploying ${agent_name} to Azure AI Foundry...${NC}"
    
    if az ml flow create --file flow.dag.yaml --resource-group "$RESOURCE_GROUP" --workspace-name "$PROJECT_NAME" 2>/dev/null; then
        echo -e "${GREEN}  ✅ ${agent_name} deployed successfully${NC}"
    else
        echo -e "${YELLOW}  ⚠️  ${agent_name} might already exist, updating...${NC}"
        # Try to update instead
        az ml flow update --file flow.dag.yaml --resource-group "$RESOURCE_GROUP" --workspace-name "$PROJECT_NAME" 2>/dev/null || true
    fi
    
    cd - > /dev/null
}

# Function to create and deploy the main workflow
create_and_deploy_main_workflow() {
    echo -e "${YELLOW}🎼 Creating and deploying main interview workflow...${NC}"
...
EOF
    ...
    cat > "flows/interview-workflow/azure_services_prompt.jinja2" << 'EOF'
system:
You are an Azure services research specialist. Research and provide comprehensive information about Azure services relevant to the given requirements.
...
Based on the case study, requirements analysis, and Azure services research, design 3 comprehensive Azure architecture solutions:
- COST-OPTIMIZED: Minimize monthly spend while meeting requirements
- PERFORMANCE-OPTIMIZED: Maximum scalability and performance  
- SECURITY-HARDENED: Enterprise security and compliance focus
...
Azure Services Research: {{azure_services}}
EOF

    cat > "flows/interview-workflow/cost_optimizer_prompt.jinja2" << 'EOF'
system:
You are an Azure Cost Optimization specialist focused on enterprise Total Cost of Ownership (TCO) analysis.

Optimize the costs for the provided architecture solution with specific savings estimates and recommendations.

user:
Architecture to optimize: {{architecture}}
EOF
...
EOF
    ...
    # Deploy to Azure AI Foundry
    cd "flows/interview-workflow"
    echo -e "${BLUE}  🔄 Deploying main workflow to Azure AI Foundry...${NC}"
    
    if az ml flow create --file flow.dag.yaml --resource-group "$RESOURCE_GROUP" --workspace-name "$PROJECT_NAME" 2>/dev/null; then
        echo -e "${GREEN}  ✅ Main workflow deployed successfully${NC}"
    else
        echo -e "${YELLOW}  ⚠️  Main workflow might already exist, updating...${NC}"
        # Try to update instead
        az ml flow update --file flow.dag.yaml --resource-group "$RESOURCE_GROUP" --workspace-name "$PROJECT_NAME" 2>/dev/null || true
    fi
...
main() {
    ...
    # Create and deploy main workflow
    create_and_deploy_main_workflow
    ...
    echo "  ✅ 4 specialized agent flows deployed to Azure AI Foundry"
    echo "  ✅ Main orchestration workflow deployed to Azure AI Foundry"
    echo "  ✅ Python synthesis and performance tracking included"
    echo ""
    echo -e "${BLUE}🌐 Access your flows at:${NC}"
    echo "  $PROJECT_ENDPOINT"
    echo ""
    echo -e "${BLUE}🧪 Test the main workflow:${NC}"
    echo "  az ml flow test --name interview-workflow --inputs case_study='Your case study text here' --resource-group $RESOURCE_GROUP --workspace-name $PROJECT_NAME"
    echo ""
...
echo -e "${GREEN}🚀 Your Azure AI Foundry flows are now deployed and ready to use!${NC}"