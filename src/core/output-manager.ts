...
export class OutputManager {
  ...
  async saveAnalysis(
    ...
    // Save original case study text
    const caseStudyFile = join(this.outputDir, caseStudyFolder, 'original-case-study.md');
    await fs.writeFile(caseStudyFile, caseStudyText, 'utf-8');
    ...
    // Save JSON metadata for tooling
    ...
    if (fullMetadata.agentMetrics) {
      await this.savePerformanceReport(fullMetadata, caseStudyFolder);
    }

    // Save agent debug logs if available
    await this.saveAgentDebugLogs(fullMetadata, caseStudyFolder);

    // Automatically generate quick summary for interview prep
    await this.saveQuickSummary(analysis, fullMetadata, caseStudyFolder);
...
- Status: ${metrics.status || 'unknown'}`;
  ...
  // Save individual agent debug logs for debugging and optimization
